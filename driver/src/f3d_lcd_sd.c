...
void f3d_lcd_fillScreen_slow(uint16_t color) {
  uint8_t x,y;
...
void f3d_lcd_drawRect(uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint16_t color){
  if ((x >= ST7735_width) || (y >= ST7735_height)) return;
  uint8_t pixel_idx;
  uint8_t pixel_arr_count;
  uint16_t pixel_arr[width];  
  for (pixel_idx = 0; pixel_idx < width; pixel_idx++) pixel_arr[pixel_idx] = color;
  f3d_lcd_setAddrWindow(x, y-height, x + width-1, y + height-1, MADCTLGRAPHICS);
  for (pixel_arr_count = 0; pixel_arr_count < height; pixel_arr_count++) {
    f3d_lcd_pushColor(pixel_arr, width);
  }
}
...
// draws horizontal line on screen for at given (x,y) with given width
void f3d_lcd_drawHLine(uint8_t x, uint8_t y, uint8_t width, uint16_t color) {
  uint8_t xOff;
  for (xOff= 0; xOff < width; xOff++) {
    f3d_lcd_drawPixel(x+xOff, y, color); 
  }
}

// draws vertical bar on screen for at given (x,y) with given height
// TODO: rename to drawVBar
void f3d_lcd_drawRect2(uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint16_t color){
  uint8_t yOff;
  for (yOff = 0; yOff < height; yOff++) {
    f3d_lcd_drawHLine(x, y-yOff, width, color);
  }
}

// draws horizontal bar scaled with given val and overrides previous bar, if any
// positive and negative vals are drawn with different colors
void f3d_lcd_drawDynamicHBar(float val, uint8_t x, uint8_t y, uint8_t width, uint8_t htOff, uint16_t col) {
  if (val < 0) {
    val *= -1;
    col = WHITE;
  }
 
  uint8_t barHeight = (uint8_t)val;
  // draw bar at height
  f3d_lcd_drawRect(x, y, width, barHeight, col);
  // delete previous bar higher than barHeight
  f3d_lcd_drawRect(x, y-barHeight, width, y-barHeight-htOff, BACKGROUND);
}
